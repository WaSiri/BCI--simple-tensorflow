...
#data_train_data = (np.reshape(data_train_data,(-1,140))).transpose()
#for pos in range(len(data_train_label)):
#    if data_train_label[pos] == [1]:
#        print(data_train_label[pos])
#        data_train_label[pos] = [0,1]
#    else:
#        dadata_train_label[pos] = [0,1]
...
#data_train_data = data_train_data[:,-10:]
data_train_data = data_train_data[:,:130].transpose()
...
for pos in range(len(orgin_data_train_label)):
    ...
    if label == [1]:
        ...
        data_train_label[pos]=[0,1]
data_train_label = data_train_label[:130,:]


ds = DataSet(data_train_data,data_train_label,130)
...
with tf.Session() as sess:
    ...
    for i in range(2000):
        batch = ds.next_batch(20)  
        if i % 20 == 0:
            train_accuracy = accuracy.eval(feed_dict = {x:batch[0], y_:batch[1], keep_prob:1.0 })
            print('step %d, training accuracy %g' % (i, train_accuracy))
        train_step.run(feed_dict = {x:batch[0], y_:batch[1], keep_prob:0.5 })
    saver.save(sess, 'D:/CSU/program/Python/Practice/Brain_cognition/Brain_cognition/model.ckpt')
